...
const Instationary = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios({
          method: 'post',
          url: `${baseUrl}/api/home-product?=3`,
          headers: {
        ...
        startTransition(() => {
          if (response.data && Array.isArray(response.data.data)) {
            const cartArray = Array.from(response.data.data[2]?.products || []);
            setTitle(response.data.title || 'Stationary Section');
            setCarts(cartArray);
            setLoading(false);
          } else {
  ...
  const handleImageClick = (e, cart) => {
    e.stopPropagation();
    window.scrollTo({ top: 0, behavior: 'smooth' });
    navigate(`/single-prod?id=${cart.id}/${cart.url_name ? cart.url_name.replace(/\s+/g, '-') : ''}`);
  };
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center justify-center pt-8 space-x-4">
            ...
            />
            <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-center whitespace-nowrap">Wedding Stationary</h1>
            <img
              src={hborder}
              alt="left border"
              className="h-12 w-32 sm:w-48 md:w-64 lg:w-80 max-sm:hidden"
          ...
          <div className="container mx-auto py-8 px-2">
            ...
            <div className="mt-8 flex justify-center">
              <Link
                to="Stasubcategory/14/Wedding-Stationary"
